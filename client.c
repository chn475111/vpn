...
#define SERVER_IP "192.168.11.251"
#define SERVER_PORT 443

#define TUN_NAME "tun0"
#define TUN_IP "10.10.1.2"
#define TUN_MASK "255.255.0.0"

#define PEER_SUBNET "192.168.12.0"
...
int main(int argc, char *argv[])
{
    ...
    rv = tcp_connect(net_fd, SERVER_IP, SERVER_PORT);
    if(rv < 0)
    {
        fprintf(stderr, "%s %s:%u - %d:%s\n", __FUNCTION__, __FILE__, __LINE__, errno, strerror(errno));
        goto ErrP;
    }
    fprintf(stdout, "%s %s:%u - 0, tcp_connect %s:%hu\n", __FUNCTION__, __FILE__, __LINE__, SERVER_IP, SERVER_PORT);
        ...
        for(i = 0; i < nfds; i++)
        {
            if(fds[i].revents & POLLIN)
            {
                if(fds[i].fd == tun_fd[0])
                {
                    ...
                    if(rv != tun_length)
                    {
                        fprintf(stderr, "%s %s:%u - 2, tcp_send fd:%d %d:%s\n", __FUNCTION__, __FILE__, __LINE__, net_fd, errno, strerror(errno));
                        goto ErrP;
                    }
                #ifdef __DEBUG__
                    fprintf(stdout, "%s %s:%u - 2, tcp_send %d:%s\n", __FUNCTION__, __FILE__, __LINE__, tun_length, tun_buffer);
                    PRINT_HEX(tun_buffer, tun_length);
                #endif
                }
                else if(fds[i].fd == net_fd)
                {
                    ...
                    if(net_length <= 0)
                    {
                        fprintf(stderr, "%s %s:%u - 3, tcp_recv fd:%d %d:%s\n", __FUNCTION__, __FILE__, __LINE__, net_fd, errno, strerror(errno));
                        goto ErrP;
                    }
                #ifdef __DEBUG__
                    fprintf(stdout, "%s %s:%u - 3, tcp_recv %d:%s\n", __FUNCTION__, __FILE__, __LINE__, net_length, net_buffer);
                #endif
                    ...
                    if(rv != net_length)
                    {
                        fprintf(stderr, "%s %s:%u - 4, tun_write fd:%d %d:%s\n", __FUNCTION__, __FILE__, __LINE__, tun_fd[0], errno, strerror(errno));
                        goto ErrP;
                    }
                #ifdef __DEBUG__
                    fprintf(stdout, "%s %s:%u - 4, tun_write %d:%s\n", __FUNCTION__, __FILE__, __LINE__, net_length, net_buffer);
                    PRINT_HEX(net_buffer, net_length);
                #endif
                }
            }
